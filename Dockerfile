...
RUN mkdir /wrf-exec
RUN ln -s /home/cuahsi/wrf_hydro_nwm/trunk/NDHMS/RUN/wrf_hydro.exe /wrf-exec/wrf_hydro.exe
RUN chown -R 1000:1000 /wrf-exec

ADD ./run-wrf.sh /srv/run-wrf.sh
RUN chmod +x /srv/run-wrf.sh
